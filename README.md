...
- 여러 행·열의 표라도 한 셀에 여러 줄 명령이 모여 있으면 셀별 코드 블록으로 펼칩니다.
  원래 행·열 위치를 표시하며, 단순 비교표와 짧은 명령/설명 표는 유지합니다.
  중첩 표는 내부 행을 부모 표에서 다시 출력하지 않습니다. 주석·들여쓰기·파이프 문자는
  코드 안에 보존하며, 이미지·하이퍼링크를 코드 텍스트로 삼키지 않습니다.
  수동 교정으로 지정한 가로 비교표는 해당 교정 규칙을 유지합니다.

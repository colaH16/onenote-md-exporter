...
로컬 curation 설정의 `layout` 항목에 `"history": true`를 주면 `block`,
`blocks`, `table`, `markdown` 모두 해당 항목만 히스토리로 감쌉니다.
`historyMarker`로 경고 설명을 지정할 수 있습니다. 개인 페이지 ID와 실제
노트별 설정은 `.local-config/curation.json`에만 보관합니다.

```markdown
<!-- rag-priority: fallback -->
...
RAG가 종료 표시(`normal`)를 지원해야 다음 현재 섹션을 올바로 분류합니다.
페이지 전체가 `old`인 경우에는 이 표시로 현재 문서로 승격되지 않습니다.
제목에 history가 있다고 자동 판정하지 않습니다(명령어 설명일 수도 있음).
이미 SilverBullet에서 편집한 문서를 고치려고 전체 `--replace` 변환을
실행하지 마세요. 필요한 표지만 원문에 삽입하고, 재변환용 로컬 설정도
함께 갱신하면 됩니다.
